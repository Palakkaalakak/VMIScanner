...
| The model called Apple a "competitor" of AbbVie / computed "final 7" in its own arithmetic but printed "— 6/10" in the verdict | Two separate bugs. (1) Benchmark bleed: the MA/AAPL/MSFT anchor block injected into every prompt made the model treat the benchmarks as rivals. The block now says explicitly: they are a MEASURING STICK, never competitors — name actual industry rivals when comparing. (2) Copy failure: the model did the arithmetic RIGHT (final 7) then printed a different digit in the verdict — and the old backstop could only clamp DOWN, so it punished the copy error twice (6→5). Fixed: enforce_calibration v3 RECOMPUTES the score forward from the source scores the model actually wrote (same compute_arithmetic as training), in either direction — when sources parse, the printed verdict digit is ignored entirely, so a copy slip costs nothing. The rubric's SCORE ARITHMETIC template also lost its ambiguous "<none / eroding>" placeholders (the model copied the slash form verbatim) — one option per slot now | `git pull`, re-evaluate — enforcement works immediately. The saved answer shows the corrected verdict; 📐 explains the correction with the full arithmetic |
| The model STILL prints a wrong score even after the calibration retrain | ROOT CAUSE found: the trained output format put "MOAT VERDICT: … — N/10" on LINE 1, so the model had to commit to a score BEFORE writing the five source scores — an autoregressive model cannot average numbers it hasn't generated yet. No amount of retraining fixes that ordering. FIXED structurally: the output format is now arithmetic-first — sources → tests → reasoning → **SCORE ARITHMETIC** (the work: average, rounding rule, redundancy cap, decay) → **MOAT VERDICT** (copies the final number) → ACTION. The rubric, build_dataset.py (gold + contrastive) and teach_calibration.py all emit/teach this order, so the score token is generated AFTER the numbers that determine it | `git pull`, then retrain so the model learns the new order: eject model → `python -m ai_moat.build_dataset` (rebuilds gold/contrastive arithmetic-first) → `python ai_moat/teach_calibration.py` (now 2 epochs — it's an order change, not a number tweak; `--dry-run` to preview) → `python ai_moat/quantize_model.py` → import. The dashboard's code enforcement stays as a backstop, but after this retrain the model's own printed number should match |
| Now it's UNDERESTIMATING AAPL — and the model still prints its own wrong score in the answer text | Two real complaints, both fixed. (1) The round-down floor was too blunt: AAPL with sources 9,8,9,8,9 (avg 8.6) got floored to 8 even with zero decay. New rule: when ALL FIVE sources are ≥8 AND the decay check is clean (answer verdict + scanner margin trend both), the average rounds to the NEAREST integer instead — 8.6 → 9. A fully-redundant non-decaying moat is exactly what the 9-10 shelf means; the floor exists to stop inflation, not to pull true Heavenly-Queens off their own shelf. ABBV still lands 7 (round-down + scanner-measured −6.1pp decay penalty — even when the model's text dodges the word "decaying", the scanner evidence catches it). (2) The dashboard now REWRITES the "MOAT VERDICT: … — N/10" line in the saved answer to the enforced score, so the wrong printed number never reaches your screen; the model's raw number is kept in `model_score`. The rubric also gained mandatory test 7: name the industry and the company's market position (dominant / top-2 / one of many) — dominance caps or supports the scale & network sources, no invented percentages | `git pull`, delete affected saved evals, re-evaluate. If you already ran teach_calibration.py before this fix, re-run it (eject model → `python ai_moat/teach_calibration.py` → `python ai_moat/quantize_model.py` → import) — it trains on answers rescored by the SAME code, so it now teaches the corrected arithmetic |
